...
const securityConfig = {
  ...
  storage: {
    ...
    keyManagement: 'Azure Key Vault', // または同等のKMS
...
function detectProhibitedContent(input: string): boolean {
  return prohibitedContent.some(term => input.includes(term));
}
```
...
```typescript
// ✅ AI サービス選定チェックリスト
interface AIServiceRequirements {
  // データ取り扱い
  dataHandling: {
    noPromptLearning: true;       // プロンプトを学習に使わない
    optOutAvailable: true;        // オプトアウト設定可能
    dataResidency: 'japan';       // 日本国内でのデータ処理
    dataRetentionDays: number;    // データ保持期間の明示
  };
  // セキュリティ認証
  certifications: {
    ismap: boolean;               // ISMAP登録
    iso27001: boolean;            // ISO 27001
    iso27017: boolean;            // ISO 27017 (クラウドセキュリティ)
    iso27018: boolean;            // ISO 27018 (クラウド個人情報)
  };
  // 契約条件
  contract: {
    sla: number;                  // SLA (%)
    supportLanguage: 'japanese';
    incidentResponse: number;     // インシデント対応時間
  };
}
```
...
- [ ] 本番環境でデバッグモードが無効か
- [ ] クラウドサービスがISMAP登録済みか
- [ ] WAF（Web Application Firewall）が設定されているか
...
| 認証 | 説明 | 必須度 |
|------|------|--------|
| ISMAP | 政府情報システムセキュリティ評価制度 | 推奨 |
| ISO 27001 | 情報セキュリティマネジメント | 必須 |
| ISO 27017 | クラウドサービスセキュリティ | 推奨 |
| ISO 27018 | パブリッククラウドの個人情報保護 | 推奨 |
| SOC 2 Type II | サービス組織の統制 | 推奨 |

### 6.2 Azure サービス確認

Memoriaで使用するAzureサービスの確認：

```bash
# 使用サービスの認証状況
Azure Static Web Apps  → ISMAP登録済み ✅
Azure App Service      → ISMAP登録済み ✅
Azure PostgreSQL       → ISMAP登録済み ✅
Azure OpenAI           → ISMAP登録済み ✅
Azure AI Search        → ISMAP登録済み ✅
```
...
- [ ] セキュリティポリシー文書が整備されているか
- [ ] インシデント対応手順が文書化されているか
- [ ] 定期的な脆弱性診断を実施しているか
- [ ] アクセスログの定期レビューを実施しているか
...
- [文部科学省 生成AI利用ページ](https://www.mext.go.jp/a_menu/other/mext_02412.html)
